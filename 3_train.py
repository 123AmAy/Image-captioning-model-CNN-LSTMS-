...
dataset_images = BASE_PATH + "/Flicker8k_Dataset/"
dataset_text = BASE_PATH + "/Flickr8k_text/"

FEATURES_PATH = BASE_PATH + "/features.p"
DESC_PATH = BASE_PATH + "/descriptions.txt"

# MODEL_PATH = BASE_PATH + "/model_7.h5"   # or model_9.h5 (better)
...
# 2048 feature vector
# features3 = extract_features(dataset_images)
# dump(features3, open("features3.p","wb"))

features3 = load(open(BASE_PATH+"/features3.p","rb"))
...
def load_features(photos):
    ...
    all_features = load(open("features3.p","rb"))    #selecting only needed features
...
train_descriptions = load_clean_descriptions(BASE_PATH + "/descriptions.txt", train_imgs)
...
tokenizer5 = create_tokenizer(train_descriptions)
dump(tokenizer5, open('tokenizer5.p', 'wb'))
vocab_size = len(tokenizer5.word_index) + 1
print(vocab_size)
...
dataset = data_generator(train_descriptions, features3, tokenizer5, max_length)
...
# import shutil
# shutil.rmtree("models2")
os.makedirs(BASE_PATH + "/models8", exist_ok=True)
s = 1

for i in range(s , s+epochs):
    dataset = data_generator(train_descriptions, train_features, tokenizer5, max_length)
    model.fit(dataset, epochs = 20, steps_per_epoch=steps, verbose=1)
    model.save(BASE_PATH + "/models8/model_" + str(i) + ".h5")

import shutil

shutil.make_archive("models8", 'zip', BASE_PATH + "/models8")